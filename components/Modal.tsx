...
function Modal() {
  ...
  // *******************************
  // get Trailer information anytime user choose different video to show modal
  useEffect(() => {
    if (!movie) return;
  ...
  return (
    <MuiModal open={showModal} onClose={handleClose} className="fixed !top-7 left-0 right-0 z-50 mx-auto w-full max-w-5xl overflow-hidden overflow-y-scroll rounded-md scrollbar-hide ">
      <>
        ...
        about this movie information area */}
        <div className="flex space-x-16 rounded-b-md bg-[#181818] px-10 py-8">
          <div className="space-y-6 text-lg">
            <div className="flex items-center space-x-2 text-sm">
              <p className="font-semibold text-green-400">{movie?.vote_average * 10}% match</p>
              <p className="font-light ">{movie?.release_date || movie?.first_air_date}</p>
              <div className="flex h-4 justify-center items-center rounded border border-white/40 px-1.5 text-xs">HD</div>
            </div>
